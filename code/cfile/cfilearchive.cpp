...
// Used to fix move lib_offset from the local file header to the actual data in zip files
void cf_fix_zip_lib_offset(CFILE* cfile)
{
	if (cfile->size < sizeof(int))
		return;
	int signature = cfread_int(cfile);
	if (signature == ZIP_HEADER_MAGIC)
	{
		Assertion(zip_get_real_data_offset(cfile->fp, &cfile->lib_offset) == ZIP_OK, "zip_get_real_data_offset() did not returned ZIP_OK. Unable to get real data offset for packed zip file.");
	}
